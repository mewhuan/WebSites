                  ...
                  <h2 class="font-thin m-b">Discover <span class="musicbar animate inline m-l-sm" style="width:20px;height:20px">
                   ...
                   <section class="scrollable">
                          <ul class="list-group no-radius m-b-none m-t-n-xxs list-group-lg no-border">
                            ...
                            $stmt = $mysqli->prepare("select concerttype,user3,name3,user2,user1,name2,name1,vname,city,sellinghyperlink,type,t1.cid,timed,cname,ctime,price,cimage,uimage,rname,plan from (select t3.type as concerttype,u3.username as user3,u3.namess as name3,u2.username as user2,u1.username as user1,u2.namess as name2,u1.namess as name1,v.vname,v.city,v.sellinghyperlink,t1.type,c.cid,timediff(now(),rtime) as timed,cname,
                            ctime,price,c.image as cimage,u1.image as uimage,rl.rname
                            from recommend_type rt, recommendlist rl, users u3,users u1,users u2,user_like ul,follow f,
                            type t1,type t2,type t3,concert c,located l,venue v,concert_type ct where u3.username=poster and l.vname=v.vname and l.cid=c.cid and
                            follower='$username' and rl.cid=c.cid and c.actor=u2.username and rl.cid=rt.cid and rl.rname=rt.rname and followee=u1.username and rl.username=followee and rt.username=followee
                            and rt.rtype=t2.subtype and ul.username='$username' and ul.subtype=t1.subtype and t1.type=t2.type and ct.cid=rl.cid and ct.ctype=t3.subtype order by timed asc) t1 left outer join
                            (select * from plans_attend where username='$username') t2 on t1.cid=t2.cid");
                            $stmt->execute();
                            $stmt->bind_result($ctype,$oriuser,$oriposter,$actuser,$postuser,$actor,$poster,$vname,$city,$sell,$type,$cid,$time,$cname,$ctime,$price,$cimage,$uimage,$rname,$plan);
                            while($stmt->fetch()){
                              ?>
                            <li class="list-group-item">
                              ...
                              <a href="#" class="clear">
                                <?php $times = explode(":", (string)$time); 
                                  ...
                                  $rname1 = str_replace(' ','%20',$rname);
                                  echo '<font color="red" size="2" onclick=window.location.href="userinfo.php?rname='.$rname1.'&user='.$postuser.'">List: '.$rname.'</font>';
                                  echo '<font color="grey" size="2"> List type: '.$type.'</font><br>';
                                  echo '<font color="grey" size="2">Type: '.$ctype.'</font><br>';
                                  ...
                                  echo '<font color="grey" size="2">Buy now: '.$sell.'</font><br>';
                                  echo '<font color="red" size="2" onclick=window.location.href="userinfo.php?user='.$oriuser.'">Original Post By: '.$oriposter.'</font><br><br>';
                                  $now = time();
