                  ...
                  <h2 class="font-thin m-b">Discover <span class="musicbar animate inline m-l-sm" style="width:20px;height:20px">
                   ...
                   <section class="scrollable">
                          <ul class="list-group no-radius m-b-none m-t-n-xxs list-group-lg no-border">
                            ...
                            $stmt = $mysqli->prepare("select user1,user2,vname,city,sellinghyperlink,type,t1.cid,timed,cname,ctime,price,name1,name2,cimage,uimage,plan 
                            from (select u1.username as user1,u2.username as user2,v.vname,v.city,v.sellinghyperlink,t.type,c.cid, timediff(now(),posttime) as timed,
                            cname,ctime,price,u1.namess as name1,u2.namess as name2,c.image as cimage,u1.image as uimage
                            from follow f,users u1,concert_type ct, type t,concert c,located l,venue v,users u2
                            where follower='$username' and followee=u1.username and followee=poster and c.cid=l.cid and l.vname=v.vname
                            and c.cid=ct.cid and ct.ctype=t.subtype and actor=u2.username order by timed asc) t1 left outer join
                            (select * from plans_attend where username='$username') t2 on t1.cid=t2.cid");
                            $stmt->execute();
                            $stmt->bind_result($postuser,$actuser,$vname,$city,$sell,$type,$cid,$time,$cname,$ctime,$price,$poster,$actor,$cimage,$uimage,$plan);
                            while($stmt->fetch()){
                              ?>
